...
class PurchaseForm(forms.Form):
	quantity = forms.IntegerField(label='Quantity', 
		validators=[
			MaxValueValidator(100, message="Max quanlity for purcahse 100"),
			MinValueValidator(1, message="Min quanlity for purcahse 1")
		])
	voucher = forms.CharField(label='Voucher code', max_length=10, required=False)
			...
			if voucher:
				...
				if item and voucher.verify_eligible_type(item.item_type):
					if voucher.verify_max_redeem():
						...
						self.add_error('voucher', "Voucher code has been fully redeemed")
				else:
					self.add_error('voucher', "Voucher do not apply to this item")
			else:
